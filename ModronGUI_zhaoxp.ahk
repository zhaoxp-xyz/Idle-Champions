...
SafetyCheck() 
{
    While (Not WinExist("ahk_exe IdleDragons.exe")) 
    {
        ...
        {
            GuiControl, MyWindow:, I can't readbase pointer
            sleep, 3000
...
CloseIC()
{
    while (ReadChampBenchedByID(1,, 58) == 1 AND ElapsedTime < 30000)
    {
...
SetFormation()
{
    ...
    else if (!ReadQuestRemaining(1) AND ReadTransitioning(1) AND gLevel_Number < gAreaLow)
    {
        ...
        ; while (ElapsedTime < 3000 AND !ReadQuestRemaining(1))
        ; {
...
LoadingZoneOne()
{
    ...
    ElapsedTime := 0
    GuiControl, MyWindow:, gloopID, Confirming Zone Load
    while (ReadChampBenchedByID(1,, 58) != 1 AND ElapsedTime < 3000)
    {
        DirectedInput("e{F5}e")
        ElapsedTime := UpdateElapsedTime(StartTime)
        UpdateStatTimers()
...
StackRestart()
{
    ...
    while (ReadChampBenchedByID(1,, 47) != 1 AND ElapsedTime < 3000)
    {
        DirectedInput("{w}")
        ElapsedTime := UpdateElapsedTime(StartTime)
        UpdateStatTimers()
...
CheatengineSend(s)
{
    ...
    ;~ {
        ;~ Send, %s%
		;~ ControlFocus,, ahk_exe cheatengine-x86_64-SSE4-AVX2.exe
	;~ }
    ControlSend,, {Blind}%s%, ahk_exe cheatengine-x86_64-SSE4-AVX2.exe
...
SwapOutBriv()
{
	...
	{
		DirectedInput("e")
        GuiControl, MyWindow:, gloopID, "setformation E kkkkkkkkkkkkkkkkkkk"
		Return 1
	}
	DirectedInput("q")
    GuiControl, MyWindow:, gloopID, "setformation Q kkkkkkkkkkkkkkkkkkk"
