...
Gui, Tab, Settings
Gui, MyWindow:Add, Text, x15 y30 w370, Seats to level with Fkeys:
Loop, 12
{
...
Gui, MyWindow:Font, cBlue
Gui, MyWindow:Add, Text, x15 y+10 , Briv skip
Gui, MyWindow:Add, DDL, vgBrivSkip x+5  w35 gBriv_Changed, 1||2|3|4|5|6|7|8|9
Gui, MyWindow:Add, Checkbox, vg100PercentBriv %g100PercentBriv% x+5 gBriv_Changed, 100`%
...
Gui, MyWindow:Add, Text, x+5, myIC Is defaut time
Gui, MyWindow:Add, Edit, vmyjumplevel x15 y+10 w50, % gmyjumplevel
Gui, MyWindow:Add, Text, x+5, that Is myjumplevel
Gui, MyWindow:Add, Edit, vrestartgems x15 y+10 w50, % grestartgems
Gui, MyWindow:Add, Text, x+5, that Is restartgems
Gui, MyWindow:Add, Edit, vgamerestartlevel x15 y+10 w50, % ggamerestartlevel
Gui, MyWindow:Add, Text, x+5, that Is gamerestartlevel
...
SafetyCheck() 
{
    While (Not WinExist("ahk_exe IdleDragons.exe")) 
    {
        ...
        ;Sleep gGetAddress
        ;addby zhaoxp
        ;while (ReadChampBenchedByID(1,, 58))
        ;{
            ;GuiControl, MyWindow:, I can't readbase 
            ;sleep, 3000
...
CloseIC()
{
    ;while (ReadChampBenchedByID(1,, 58) == 1 AND ElapsedTime < grestarttime)
    ;{
        ;GuiControl, MyWindow:, gloopID, Briv in formation !!!
        ;Sleep, 1000
        ;DirectedInput("{w}")
        ;sleep 100
    ;}
    ...
    While (WinExist("ahk_exe IdleDragons.exe") AND ElapsedTime < grestarttime) 
    {
        Sleep 100
        ElapsedTime := UpdateElapsedTime(StartTime)
        UpdateStatTimers()
...
;not be used.because nothing to do ;-) hahahaha
CheckForFailedConv()
{
...
FinishZone()
{
    ...
    while (ReadQuestRemaining(1) AND ElapsedTime < grestarttime)
    {
...
DoDashWait()
{
    ...
    while (ReadCurrentZone(1) == 1 AND ElapsedTime < grestarttime)
    {
        ;SetFormation()
        DirectedInput("{Right}")
        ElapsedTime := UpdateElapsedTime(StartTime)
...
SetFormation()
{
    ...
    mod_number := 0
    mod_number := Mod((gLevel_Number + 1 + gmyjumplevel), 5)
    if (gAvoidBosses AND !Mod(gLevel_Number, 5))
    {
        GuiControl, MyWindow:, gloopID, setformation E `for boss
        DirectedInput("{e}")
    }
    ;add by zhaoxp
    else if (mod_number < 1 )
    {
        GuiControl, MyWindow:, gloopID, "DefaultFormation E kkkkkkkkkkkkkkkkkkk"
        DirectedInput("{e}")
    }
    else if (mod_number >= 1 )
    {
        GuiControl, MyWindow:, gloopID, "setformation Q kkkkkkkkkkkkkkkkkkk"
        DirectedInput("{q}")
    }
    ;end add by zhaoxp
    ;change by zhaoxp
    ;else if (mod_number < 3 and !ReadQuestRemaining(1) AND ReadTransitioning(1) AND gLevel_Number < gAreaLow)
    else if (!ReadQuestRemaining(1) AND ReadTransitioning(1) AND gLevel_Number < gAreaLow)
    {
        GuiControl, MyWindow:, gloopID, setformation E
        DirectedInput("{e}")
        ; StartTime := A_TickCount
        ; ElapsedTime := 0
        ; GuiControl, MyWindow:, gloopID, ReadTransitioning
        ; while (ElapsedTime < grestarttime AND !ReadQuestRemaining(1))
        ; {
    ...
    else
    {
        ...
        if (gDefaultFormation)
        {
            GuiControl, MyWindow:, gloopID, DefaultFormation E xDefault
            DirectedInput("{e}")
        }
        else
        {
            GuiControl, MyWindow:, gloopID, DefaultFormation Q `Default
            DirectedInput("{q}")
        }
    }
}
...
LoadingZoneREV()
{
    ...
    ;while (ReadChampBenchedByID(1,, 58) != 1 AND ElapsedTime < grestarttime)
    ;{
        ;GuiControl, MyWindow:, gloopID, what `this fuck?
        ;sleep, 3000
    ...
    while (ReadChampBenchedByID(1,, 58) != 0 AND ElapsedTime < grestarttime)
    {
        DirectedInput("w{F5}w")
        ElapsedTime := UpdateElapsedTime(StartTime)
        UpdateStatTimers()
...
StackRestart()
{
    ...
    ;while (ReadChampBenchedByID(1,, 47) != 1 AND ElapsedTime < grestarttime)
    ;{
        ;DirectedInput("{w}")
        ;ElapsedTime := UpdateElapsedTime(StartTime)
        ;UpdateStatTimers()
    ...
    else if (stacks < gSBTargetStacks AND gLevel_Number > gAreaLow)
    {
        StackFarm()
    }
    ;==========================================================================
...
StackFarm()
{
    ...
    ;while (stacks < gSBTargetStacks AND gLevel_Number > gAreaLow)
    while (stacks < (gSBTargetStacks + 200) AND ElapsedTime < gSBTimeMax AND gLevel_Number > gAreaLow)
    {
    	DirectedInput("w")
        GuiControl, MyWindow:, gloopID, sstackNormall
        sleep, 100
        stacks := GetNumStacksFarmed()
        ElapsedTime := UpdateElapsedTime(StartTime)
        gLevel_Number := ReadCurrentZone(1)
        ...
        {
            GuiControl, MyWindow:, ReadHighestZoneID, % gLevel_Number
            DirectedInput("{left}")
        }
    }
    ;if ( stacks > (gSBTargetStacks + 200))
    ;{
        ;DirectedInput("{left}")
        ;while (ReadChampBenchedByID(1,, 75) != 1)
        ;{
            ;DirectedInput("{left}")
            ;DirectedInput("{q}")
            ;ElapsedTime := UpdateElapsedTime(StartTime)
...
UpdateStatTimers()
{
    ...
    gLevel_Number := ReadCurrentZone(1)
    ;if ( (dtCurrentRunTime + 1) > gSBTimeMax )
    ;{
        ;if ( (gLevel_Number - oldgLevel_Number ) = 1 )
        ;{
            ;DirectedInput("{left}")
            ;if (gDefaultFormation)
            ;{
                ;GuiControl, MyWindow:, gloopID, DefaultFormation E XDefaulte
                ;DirectedInput("{e}")
            ;}
            ;else
            ;{
...
{  
    ...
    loop 
    {
        ...
        ;if (Mod(ReadHighestZone(1), 5) AND !ReadTransitioning(1) and dontmove)
        ;{
            ;DirectedInput("{g}")
        ;}
...
ModronReset()
{
    ...
    while (ReadResetting(1) AND ElapsedTime < grestarttime)
    {
        Sleep, 250
        ElapsedTime := UpdateElapsedTime(StartTime)
        UpdateStatTimers()
...
SwapOutBriv()
{
	...
	DirectedInput("q")
    GuiControl, MyWindow:, gloopID, "setformation Q"
	return
}
